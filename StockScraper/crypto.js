...
function extractLink(html){
    ...
    for(let j=0;j<table.length;j++)
    {
        ...
        let marketCap=$(allCols[5]).text().trim();
        let volume=$(allCols[6]).text().trim();

        //console.log(`${symbol}|${name}|${lastPrice}|${MarketTime}|${change}|${percentageChange}|${volume}|${marketCap}`)

        processCrypto(descEl,name,price,change,percentageChange,marketCap,volume);
    }
            
}

function processCrypto(title,name,price,change,percentageChange,marketCap,volume)
{
